...
export interface MockTatsuState {
...
export function MockTatsu({ state }: { state: MockTatsuState }) {
  return (
    <div
...
function MockSidebar({ state }: { state: MockTatsuState }) {
  ...
  return (
    <motion.div
      animate={{
        boxShadow: highlighted
          ? '0 0 24px rgba(198, 124, 92, 0.30)'
          : '0 0 0px rgba(198, 124, 92, 0)'
      }}
    ...
    >
      ...
      <Plus
        ...
        style={highlighted ? { stroke: 'url(#tatsu-add-gradient)' } : undefined}
      />
...
function MockTerminalPanel({ state }: { state: MockTatsuState }) {
...
function ClaudeTUI({ active }: { active: MockWorktree }) {
  ...
  const repoName = 'claude-tatsu/' + active.path.replace(/^tatsu\//, '')
  return (
