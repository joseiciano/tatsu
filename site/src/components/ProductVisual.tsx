...
const BASE_WORKTREES: Omit<MockWorktree, 'status'>[] = [
  {
    ...
    path: 'tatsu/feat-onboarding',
    pr: { checks: 'success', additions: 142, deletions: 58 }
  },
  {
    id: '2',
    branch: 'fix/login-flash',
    path: 'tatsu/fix-login-flash'
  },
  {
    id: '3',
    branch: 'refactor/auth',
    path: 'tatsu/refactor-auth'
  },
  {
    id: '4',
    branch: 'chore/deps-bump',
    path: 'tatsu/chore-deps-bump'
  },
  {
    id: '5',
    branch: 'docs/api-reference',
    path: 'tatsu/docs-api-reference'
  }
]
...
function dioramaStateFor(phase: Phase, reduced: boolean): MockTatsuState {
...
function sectionOneState(progress: number): MockTatsuState {
...
function sectionTwoState(progress: number): MockTatsuState {
...
function sectionThreeState(progress: number): MockTatsuState {
  const worktrees = BASE_WORKTREES.map((w, i) => ({
    ...w,
...
function sectionFourState(progress: number): MockTatsuState {
  const worktrees = BASE_WORKTREES.map((w, i) => ({
    ...w,
...
function finalState(): MockTatsuState {
  const worktrees = BASE_WORKTREES.map((w, i) => ({
    ...w,
    status: sectionOneStatusFor(i, 0.8)
  }))
