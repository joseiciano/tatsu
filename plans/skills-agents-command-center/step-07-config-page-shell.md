...
- **REQ-009**: `Config.tsx` MUST use `useActiveBackend().id` as a page-session boundary. When the active backend changes, clear selected resource, create/edit draft, local operation feedback, open dialog state, and the per-session scanned-scope set before loading scopes from the newly active backend.
...
- **REQ-013**: The displayed rows for a scope MUST combine disk-backed refs from `harnessConfig.resources[activeTab]` with config-only refs from the latest plan at `harnessConfigScopeKey(scope)`. Deduplicate by `agentKind + id`, prefer the disk ref when both sides exist, and never create a second physical row for a Claude skill alias already represented in the current logical view.
...
- **REQ-032**: Use only existing theme utilities such as `bg-app`, `bg-panel`, `bg-panel-raised`, `bg-surface`, `text-fg`, `text-muted`, `text-dim`, `text-faint`, `text-success`, `text-warning`, `text-danger`, `text-info`, `text-accent`, and matching border tokens. Do not add Config-specific global CSS, a new font, a new color token, gradients, decorative card framing, or a runtime dependency.
...
- **CON-004**: Do not add main-process handlers, filesystem access, persistence logic, state events, reducer cases, preload wiring, or duplicated transport/domain types. Steps 2 through 6 are prerequisites and remain authoritative.
...
- [ ] **TASK-005**: Implement deterministic list derivation and rendering in `ConfigResourceList.tsx`.
  - Export a package-internal `buildConfigResourceGroups` helper for the focused test. It accepts active resource type, agent filter, normalized search query, shared resources, scoped plans, selected identity, and registry capability metadata, then produces registry-ordered groups and sorted/deduplicated rows as required by REQ-012 through REQ-015.
...
- [ ] **TASK-006**: Implement `ConfigEditor.tsx` around the existing `MonacoEditor`.
  ...
  - Ensure Cmd/Ctrl-S reaches the provided save callback only when the parent marks the editor actionable.
- [ ] **TASK-007**: Implement `ConfigSyncDialog.tsx` as the read-only Step 7 preview.
  ...
  - Render Close as the only footer action. Escape and backdrop click call `onClose`; Enter performs no action.
...
- [ ] **TASK-008**: Implement the smart component state and derived view models in `Config.tsx`.
  ...
  - Track the selected identity as exact scope plus ID, editor mode, returned ref, draft/saved content, logical create name, request/loading error state, notice, open sync plan, read request token, and a per-backend set of scopes scanned in the current page session.
- [ ] **TASK-009**: Implement initial scoped scan/comparison and active-backend reset behavior.
  - Dependency: TASK-008.
  - For each newly visible supported scope, issue `scanHarnessConfig` and, only on its success, issue the read-only `planHarnessConfigSync` comparison required by REQ-010. Use `Promise.allSettled` or independent tasks so one harness failure does not hide successful groups.
  ...
  - On active backend change, immediately invalidate pending reads and clear page-local editor/dialog state, reset the scanned-scope set, and load the new backend's visible scopes without reusing an old selection, draft, or plan object. Do not offer to keep an old-backend draft after routing has changed.
- [ ] **TASK-010**: Implement dirty-state guards, tab/filter/search behavior, and selection reads.
  ...
  - On disk-row selection, increment the read token, enter loading state, call `readHarnessConfigFile`, and apply the result only if the token and selected identity still match. Set returned content as both draft and saved baseline.
...
- [ ] **TASK-011**: Implement direct create and update handlers with exact prepare/apply sequencing.
  ...
  - Disable duplicate submission while either phase is pending. Never retain a prepared plan after cancellation, scope change, or a failed apply.
  - On success, follow REQ-022 and REQ-023: wait for the transport request, consume the refreshed mirrored inventory, select/reload a resulting ref when available, and request a new scoped comparison without mutating local inventory.
- [ ] **TASK-012**: Implement confirmed delete and read-only sync-preview handlers.
  - Dependency: TASK-010.
  - Delete shows the backup/removal confirmation, then executes the prepare/apply `deleteHarnessConfigFile` sequence from REQ-021. Clear the editor only after a successful apply result; retain the draft and error when preparation or application fails.
  - Sync always calls a fresh `planHarnessConfigSync({ scope, direction: 'sync-to-disk' })`. Show a success notice for `synced`; otherwise store the returned plan in local dialog state.
...
- [ ] **TASK-013**: Compose `ConfigTabs`, `ConfigResourceList`, `ConfigEditor`, and conditional `ConfigSyncDialog` in `Config.tsx`.
  ...
  - Add an Escape listener that routes through the dirty close guard only when the sync dialog is closed and no input/editor control has already handled the event.
...
- [ ] **TASK-014**: Create `src/renderer/components/Config/ConfigResourceList.test.ts` with focused pure-data tests for `buildConfigResourceGroups`.
  ...
  - Prove config-only plan refs appear even though scan inventory is disk-only, disk refs win same-ID deduplication, changed membership alone produces Conflict, and unrelated rows remain Synced even when the overall scope plan is conflict.
  - Prove a Claude alias retains one stable ID in the active logical view and an unsupported capability remains visible with disabled actions and its note.
...
- [ ] **TASK-016**: Smoke-test the actual renderer surface before finalizing the step.
  - Dependency: TASK-015.
  - Temporarily mount `Config` from the package barrel as the desktop root in `src/renderer/App/App.tsx`, launch `pnpm dev`, and inspect the real Electron renderer at default and increased `uiScale` values.
  - Exercise all three tabs, All and individual agent filters, search/clear, harness-group empty states, row selection/read, create-mode entry/cancel, dirty-discard cancellation, Cmd/Ctrl-S gating, and read-only sync preview. Do not confirm Create/Save/Delete against real user harness directories during this visual smoke.
  - Verify keyboard tab order, focus visibility, Escape/backdrop behavior, long absolute paths, narrow-window overflow, Monaco resizing, semantic theme contrast, and that every icon scales with `uiScale`.
  - Remove the temporary App mount after the smoke and rerun `pnpm typecheck` plus `pnpm build` so the delivered tree retains the Step 8 navigation boundary.
- [ ] **TASK-017**: Review the final change against Steps 3, 4, 6, 8, 9, and 10.
  - Dependency: TASK-016.
  - Verify capability support comes only from registry metadata; shared inventory/plans remain store-owned; file content remains request-local; all mutation requests use exact typed prepare/apply payloads; display paths are never submitted; and aliases retain service-issued IDs.
  - Verify no Config code calls sync/adopt apply or conversion methods, no navigation source changed, no new global CSS/dependency was added, and no disallowed text/icon sizing appears.
...
- **DEP-003**: [Step 2](./step-02-main-process-harness-config-service.md) provides stable file identity, resolver behavior, plan generation, prepare/apply mutation semantics, backups, stale-plan rejection, and post-apply results.
- **DEP-004**: [Step 3](./step-03-harness-capability-metadata.md) provides `HarnessConfigResourceType`, `AgentInfo.configCapabilities`, exact support/notes, and Claude alias metadata through the shared registry barrel.
- **DEP-005**: [Step 4](./step-04-shared-state-slice.md) provides the content-free `harnessConfig` slice, `HarnessConfigFileRef`, `HarnessConfigSyncPlan`, `harnessConfigScopeKey`, loading/error state, and resource/plan event semantics.
- **DEP-006**: [Step 6](./step-06-transport-request-handlers.md) provides the ten typed active-backend methods, result envelope, scan/read responses, prepare/apply request unions, scoped plan binding, authoritative rescan behavior, and renderer/main contract names.
- **DEP-007**: `src/renderer/components/MonacoEditor/` provides the controlled editor, file-path language detection, read-only option, and Cmd/Ctrl-S callback.
- **DEP-008**: `src/renderer/components/AgentIcon/`, `src/shared/agent-registry/`, `src/renderer/backend/`, and `src/renderer/store/` provide existing icons, display metadata, active-backend request routing, and mirrored state access.
- **DEP-009**: [Step 8](./step-08-navigation-entry-points.md) will mount `Config` as per-client overlay state and add Sidebar/home entry points after this package exists.
- **DEP-010**: [Step 9](./step-09-sync-conflict-ux.md) will extend `ConfigSyncDialog` and `Config.tsx` with detailed differences, fresh direction-specific plans, confirmed Sync-to-disk/Adopt-from-disk application, stale-plan recovery, and Cancel no-op behavior.
- **DEP-011**: [Step 10](./step-10-skill-command-conversion.md) will add same-harness conversion actions and generated-draft review without changing Step 7's identity or capability boundaries.
...
- **TEST-002**: Drift tests prove config-only refs are merged into the visible list, disk refs win same-ID deduplication, badges reflect per-resource membership rather than scope status, and Claude aliases retain one physical ID.
...
- **TEST-006**: The temporary real-renderer smoke in TASK-016 verifies tabs, filters, search, scoped groups, editor states, dirty guards, read-only sync preview, keyboard behavior, theme tokens, overflow, and `uiScale` icon/text scaling; the temporary App mount is absent from the delivered diff.
...
- **RISK-004**: A delayed read response can overwrite a newer selection or a selection from another backend. The request-token and selected-identity checks in REQ-016 and backend reset in REQ-009 prevent that race.
...
- **ASSUMPTION-001**: Steps 2 through 6 are implemented first or in the same branch and converge on the exact shared contracts and renderer method names declared in their plans; Step 7 does not add compatibility aliases for partially implemented prerequisites.
