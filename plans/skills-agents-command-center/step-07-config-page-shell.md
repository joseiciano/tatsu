...
- **REQ-005**: Add `useHarnessConfig()` to `src/renderer/store/store.ts` as `useAppState((state) => state.harnessConfig)`. `Config.tsx` MUST read inventory, comparisons, loading, error, and timestamps from that mirrored slice rather than maintaining a renderer-local copy of shared state.
...
- **REQ-010**: On first display of a supported `{ agentKind, resourceType }` scope during the current page/backend session, call `scanHarnessConfig({ scope })`; after a successful scan, call `compareHarnessConfig({ scope })` as the read-only comparison so drift badges can be derived from the returned `HarnessConfigComparison`. `All` scans the three visible managed harness scopes independently. Re-rendering or changing search text MUST NOT rescan an already loaded scope.
- **REQ-011**: Every read, create, update, delete, scan, and comparison request MUST use the exact selected `{ agentKind, resourceType }` scope and the typed Step 6 `ElectronAPI` method. No renderer action may broaden to every harness, use `reqLocal`, submit an absolute path, submit a destination harness, or call a generic filesystem API.
...
- **REQ-013**: The displayed rows for a scope MUST combine disk-backed refs from `harnessConfig.resources[activeTab]` with every public ref for that exact scope in the comparison from `harnessConfig.comparisons[harnessConfigScopeKey(scope)]`: `diskOnly`, `configOnly`, and both `disk` and `config` sides of `changed`. Deduplicate by `agentKind + id`, prefer the disk-backed ref when both sides exist, exclude refs outside the active logical scope, and never create a second physical row for a Claude skill alias already represented in the current logical view.
...
- **REQ-015**: Every row MUST show exactly one inline drift badge: `Synced`, `Disk only`, `Config only`, or `Conflict`. Derive `Conflict` from membership in `comparison.changed`, `Disk only` from `comparison.diskOnly`, `Config only` from `comparison.configOnly`, and `Synced` otherwise. A scope-level `conflict` status MUST NOT incorrectly mark every unchanged row as conflicting.
...
- **REQ-019**: Clicking a group's Create action MUST enter create mode for that exact supported scope, clear any selected resource, and initialize empty logical-name and content drafts. Submitting the editor's `Create` action MUST call `prepareHarnessConfigCreate` with `{ scope, name, content }`, then call `createHarnessConfigFile` with `{ scope, planId, confirmed: true }` — applying the plan returned by the matching prepare call — only when preparation succeeds. The explicit Create submission is the user confirmation for a brand-new file.
- **REQ-020**: Saving an existing disk-backed resource MUST require an explicit `window.confirm` that identifies the file and states that Tatsu creates a backup before overwrite. After confirmation, call `prepareHarnessConfigUpdate` with the payload `{ scope, id, content }`, then apply the returned plan through `updateHarnessConfigFile` with `{ scope, planId, confirmed: true }`.
- **REQ-021**: Deleting an existing disk-backed resource MUST require an explicit `window.confirm` that identifies the file and states that Tatsu creates a backup before removal. After confirmation, call `prepareHarnessConfigDelete` with `{ scope, id }`, then apply the returned plan through `deleteHarnessConfigFile` with `{ scope, planId, confirmed: true }`.
- **REQ-022**: Create, update, and delete MUST be non-optimistic. Do not insert, patch, or remove shared inventory locally. Treat each `HarnessConfigRequestResult` envelope as authoritative for the initiating action and let Step 6's post-apply rescan update the mirrored slice. After successful direct mutation, request a fresh read-only comparison for each affected logical scope so badges do not retain the pre-mutation comparison.
...
- **REQ-027**: The Sync action MUST always request a fresh comparison via `compareHarnessConfig({ scope })` for the exact group scope. If the returned comparison has `status: 'synced'`, keep the dialog closed and render an inline success notice. If it has drift, open `ConfigSyncDialog` with that comparison. Step 7's dialog is a read-only preview that displays scope, status, and counts for disk-only/config-only/changed entries and provides only Close/Cancel behavior; Step 9 adds the confirmed Sync-to-disk and Adopt-from-disk outcomes and detailed conflict review.
- **REQ-028**: Closing or dismissing `ConfigSyncDialog` MUST be a no-op for disk and Tatsu config. It MUST never call `syncHarnessConfigToDisk` or `adoptHarnessConfigFromDisk` in this step, and pressing Enter MUST NOT trigger an apply.
...
- **SEC-001**: Renderer values are never filesystem authority. The renderer may submit only scope, stable resource ID, logical create name, draft content, prepared plan ID, and confirmation as allowed by Step 6.
...
- **CON-003**: Do not call or render `prepareHarnessConfigCommandFromSkill` or `prepareHarnessConfigSkillFromCommand` in this step. Step 10 owns conversion generation, generated-draft review, and alias no-op behavior.
...
- [ ] **TASK-001**: Create `src/renderer/components/Config/types.ts` using canonical imports from `src/shared/agent-registry` and `src/shared/state/harness-config`.
  ...
  - Define render-ready `ConfigResourceRow` and `ConfigResourceGroup` view models containing the shared ref/comparison/capability objects plus derived badge, selected, disabled, and explanatory values; do not restate any shared domain object field-by-field.
...
- [ ] **TASK-007**: Implement `ConfigSyncDialog.tsx` as the read-only Step 7 preview.
  - Render the exact scope's harness display name, resource label, comparison status, and counts for `diskOnly`, `configOnly`, and `changed`.
...
- [ ] **TASK-008**: Implement the smart component state and derived view models in `Config.tsx`.
  ...
  - Keep only per-client UI state locally. Derive resource groups with `buildConfigResourceGroups`; do not copy shared resource or comparison maps into `useState`.
  - Track the selected identity as exact scope plus ID, editor mode, returned ref, draft/saved content, logical create name, editor read/mutation busy and safe-error state, per-scope scan/comparison busy-error-notice maps keyed by `HarnessConfigScopeKey`, open sync comparison, read request token, a backend-session generation ref, and a per-backend set of scopes scanned in the current page session.
- [ ] **TASK-009**: Implement initial scoped scan/comparison and active-backend reset behavior.
  - Dependency: TASK-008.
  - For each newly visible supported scope, issue `scanHarnessConfig` and, only on its success, issue the read-only `compareHarnessConfig({ scope })` comparison required by REQ-010. Use `Promise.allSettled` or independent tasks so one harness failure does not hide successful groups; do not issue either request for an unsupported capability.
  ...
  - On active backend change, increment the backend-session generation before clearing page-local editor/dialog/request state and the scanned-scope set. Capture backend ID plus generation at the start of scans, comparisons, reads, and mutations; after every `await`, ignore the result and stop the chain when either no longer matches. Load the new backend's visible scopes without reusing an old selection, draft, feedback, or comparison object.
...
- [ ] **TASK-011**: Implement direct create and update handlers with exact prepare/apply sequencing.
  - Dependency: TASK-010.
  - Create requires a non-empty trimmed logical name, passes that trimmed name to the backend, and sends draft content byte-for-byte without trimming or normalization; empty content is allowed because the Step 6 contract accepts any string. Then execute the two-step sequence from REQ-019: `prepareHarnessConfigCreate` followed by the `createHarnessConfigFile` mutation channel with `{ scope, planId, confirmed: true }`.
  - Update is a no-op when the draft equals the saved baseline. Otherwise show the backup confirmation from REQ-020, execute the two-step sequence from REQ-020 (`prepareHarnessConfigUpdate` then `updateHarnessConfigFile`), and stop immediately on a failed result envelope.
  - Disable duplicate submission while either step is pending. Capture exact scope, resource ID/name, backend ID, and backend generation before the prepare call; after every response envelope, re-check the session before updating feedback or issuing the mutation call. Never retain a prepared plan after cancellation, scope change, backend change, or a failed apply.
...
- [ ] **TASK-012**: Implement confirmed delete and read-only sync-preview handlers.
  - Dependency: TASK-010.
  - Delete shows the backup/removal confirmation, then executes the two-step sequence from REQ-021 (`prepareHarnessConfigDelete` followed by the `deleteHarnessConfigFile` mutation channel). Apply the same backend-session checks before the mutation call and before consuming either response. Clear the editor only after a successful apply result; retain the draft and safe envelope error when preparation or application fails in the same session.
  - Sync always captures the exact group scope and backend session, then calls a fresh `compareHarnessConfig({ scope })`. Ignore stale-session results; otherwise show a success notice for `synced` or store the returned comparison in local dialog state.
...
- [ ] **TASK-014**: Create `src/renderer/components/Config/ConfigResourceList.test.ts` with focused pure-data tests for `buildConfigResourceGroups`.
  ...
  - Prove refs from `diskOnly`, `configOnly`, and both sides of `changed` appear even when absent from scan inventory; disk refs win same-ID deduplication; changed membership alone produces Conflict; and unrelated rows remain Synced even when the overall scope comparison is conflict.
...
- [ ] **TASK-017**: Review the final change against Steps 3, 4, 6, 8, 9, and 10.
  - Dependency: TASK-016.
  - Verify capability support comes only from registry metadata; shared inventory/comparisons remain store-owned; file content remains request-local; all mutation requests use the exact typed prepare and mutation channel payloads; display paths are never submitted; aliases retain service-issued IDs; and every async chain stops when its captured backend session becomes stale.
...
- **ALT-003**: Expose one global Sync button for all harnesses in the active tab. Rejected because every comparison and mutation is scoped by both `agentKind` and `resourceType`, and first-version sync requires separate confirmation per harness.
...
- **ALT-005**: Hide config-only resources because they are absent from disk scan results. Rejected because they are real persisted Tatsu config, must carry a Config only badge, and are necessary for an accurate drift preview even though Step 6 cannot read their content for editing.
...
- **DEP-001**: [implementation-details.md](./implementation-details.md) defines the feature goal, three resource views, disk-versus-Tatsu-config model, backup caveat, and the canonical Claude skills/commands alias caveat: a Claude skill and its Commands entry are one physical resource with one stable identity and path, surfaced as two logical views.
...
- **DEP-005**: [Step 4](./step-04-shared-state-slice.md) is a not-yet-implemented prerequisite that must add the content-free `harnessConfig` slice with `resources`, `comparisons`, `loading`, `error`, `lastScannedAt`, and `lastSyncedAt`; export `ManagedHarnessKind`, `HarnessConfigScope`, `HarnessConfigFileRef`, `HarnessConfigComparison`, and `harnessConfigScopeKey`; and wire the slice into `AppState`, events, reducers, initial state, and wire-snapshot merging. None of those symbols exist in the current tree.
- **DEP-006**: [Step 5](./step-05-persist-tatsu-managed-config.md) is a not-yet-implemented prerequisite that must persist the canonical Tatsu config records consumed by Step 6 and represented as config-side refs in comparisons; Step 7 does not read persisted config directly.
- **DEP-007**: [Step 6](./step-06-transport-request-handlers.md) is a not-yet-implemented prerequisite that must add the fifteen one-object `ElectronAPI` methods and active `buildBackend` mappings, with separate prepare and mutation channel contracts for direct mutations. Step 7 uses scan `{ scope }`, read `{ scope, id }`, compare `{ scope }`, the prepare channels (`prepareHarnessConfigCreate`/`prepareHarnessConfigUpdate`/`prepareHarnessConfigDelete`), and the mutation channels (`createHarnessConfigFile`/`updateHarnessConfigFile`/`deleteHarnessConfigFile` accepting `{ scope, planId, confirmed }`), each returning the canonical `Promise<HarnessConfigRequestResult<T>>`; it MUST NOT create fallback methods if those contracts are absent.
...
- **TEST-002**: Drift tests prove refs from every comparison difference collection are merged into the visible list, disk refs win same-ID deduplication, badges reflect per-resource membership rather than scope status, out-of-scope refs are excluded, and Claude aliases retain one physical ID.
...
- **RISK-001**: Step 4's scanned resource arrays contain disk inventory, so config-only resources would disappear if the renderer used only `resources`. REQ-013 requires merging refs from the latest scoped comparison without copying content into state.
- **RISK-002**: Auto-comparisons create process-local comparison snapshots that are never applied. Limit comparisons to first scope display, direct-mutation refresh, and explicit Sync clicks; always request a fresh comparison on Sync and never assume an older preview remains applicable.
...
- **RISK-005**: A config-only Tatsu config record cannot be edited because Step 6 intentionally keeps its content out of shared state and exposes read-by-disk-ID only. Step 7 renders it read-only; Step 9's confirmed sync/adopt flow is the supported resolution path.
