...
describe('installHooks', () => {
  ...
  it('emits Tatsu-owned Stop events with session IDs but no transcript paths', async () => {
    installHooks()
    const content = fsState.files.get(EXTENSION_PATH) as string
    expect(content).toContain('Auto-generated by Tatsu')
    expect(content).toContain('harness-pi-extension')
    expect(content).toContain('/tmp/harness-status')

    // Stop events are gated on the terminal id so the fallback still works.
    expect(content).toMatch(
      /process\.env\.HARNESS_TERMINAL_ID \|\| process\.env\.CLAUDE_HARNESS_ID/
    )

    const stopEmits = content.match(/await emit\('Stop', \{ session_id: sessionId \}\)/g) ?? []
    expect(stopEmits).toHaveLength(2)
    expect(content).not.toContain('transcript_path')
  })
...
describe('buildSpawnArgs', () => {
  ...
  it('does not add --model when command already contains the short -m model flag', () => {
    const result = buildSpawnArgs({ ...base, command: 'pi -m legacy', model: 'bar' })
    expect(result).toBe('pi -m legacy')
  })
